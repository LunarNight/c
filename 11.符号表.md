...
实现该客户与实现之间共享责任的方法,是创建一个函数,该函数允许客户执行一个对符号表中每个项目的回调操作.这样的函数称为**映射函数**(由于书中编写的代码,有一个.h接口,他把对符号表的操作都封装在了里面,而映射函数就是,我在这个文件中假如一个Magin的函数,就是上面的函数,你可以通过自己编写一个函数,如上面的test,通过Magin函数传入,来对符号表进行相关操作,也就是按照你test函数里写的方法对数据进行相应处理,一般事件处理就是类似的原理).  
